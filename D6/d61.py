...
def parse_url_file(file_content):
    ...
    reg_expr_w = re.compile(
        '.*?td.*?class.*?headline.*?> *([MTOFL][åinoräö]) *20[12][0-9]-0?(1?[0-9])-0?([123]?[0-9])<.*weekin.*> *(v.*?)</', re.I)
    reg_expr_d = re.compile(
        '.*?td.*?class.*?headline.*?>([A-Z][a-zåäö]) *20[12][0-9]-0?(1?[0-9])-0?([123]?[0-9])</')
    ...
    for j, line in enumerate(lines):
        ...
        if (m != None):
            day_variable = m.group(1)
            date_variable = m.group(3) + "/"+m.group(2)
            week_variable = m.group(4)
            qq.week = week_variable
            next
        ...
        if (m != None):
            ...
            qq.week = week_variable
            qq.weekday = day_variable
            qq.date = date_variable
            qq.time = m.group(1)
...
def parse_command_line_args():
    try:
        opts, rest = getopt.getopt(sys.argv[1:], "hc:u", [
                                   "help", "check=", "update"])  # lägg till kommandoargument i lista, options KAN TA IN FLER ARGUMENTS SYS.ARGV[1:] såhär [helpcheck:update]
    ...
    for option, value in opts:
        if option in ("-h", "--help"):
            usage()
            sys.exit()
        elif option in ('--check', '-c'):  # KEY
            todo["check"] = value
        elif option in ('--update', '-u'):  # DATA
            todo["update"] = value

    return todo  # {check:update}
...
# IN
# what, dataset ()
# what är en str som skickar in key (check) från todo dict, alltså det första sys.argv kommandot
# OUT
...
def main():
    ...
    # Do something
    if 'check' in todo:  # söker efter chekc (key) i todo dict
